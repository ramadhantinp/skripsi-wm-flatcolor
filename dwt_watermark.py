...
class DWT_Watermark(Watermark):
    ...
    def __embed_sig(self, bi_int_part, frac_part, combo_neg_idx, signature):
        ...
        #jika ukuran sign > cover
        if m >= n:
            for i in range(n):
                bi_int_part[i][10] = signature[i]
        
        #jika ukuran cover > sign
        if m < n:
            rate = n//m
            for i in range(m):
                for j in range(rate):
    ...
    def inner_embed(self, B, signature):
        ...
        # hori1 = np.concatenate((LL_4, LH_4), axis=1) 
        # hori2 = np.concatenate((HL_4, HH_4), axis=1)
        # verti = np.concatenate((hori1, hori2), axis=0)
        # cv2.imshow('lowhigh passes', verti)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        bi_int_part, frac_part, combo_neg_idx, _ = self.__gene_embed_space(HH_3)
        HH_3 = self.__embed_sig(bi_int_part, frac_part, combo_neg_idx, signature)

        #yang di kurungnya lagi (HL, LH, HH 4) untuk bantu aproksimasi perhitungan nge-wavelet
